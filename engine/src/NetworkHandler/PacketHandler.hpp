...
class PacketHandler {
    ...
    public:
        void insertToReceiveQueue(const std::shared_ptr<APacket> &packet, const asio::ip::udp::endpoint &endpoint);
    ...
    public:
        std::queue<std::pair<std::shared_ptr<APacket>, asio::ip::udp::endpoint>> getReceiveQueue() const;
        std::list<std::pair<const APacket &, const asio::ip::udp::endpoint &>> getValidationList() const;
    private:
