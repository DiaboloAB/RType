...
{
    this->_creationMap[HISERVER] = [](std::vector<char> &buffer)
    { return std::make_shared<HiServerPacket>(buffer); };
    this->_creationMap[HICLIENT] = [](std::vector<char> &buffer)
    ...
    { return std::make_shared<DestroyEntityPacket>(buffer); };
    this->_creationMap[MOVEENTITY] = [](std::vector<char> &buffer)
    { return std::make_shared<MoveEntityPacket>(buffer); };
...
PacketFactory::getCreationMap() const
{
...
std::shared_ptr<APacket> PacketFactory::createPacketFromBuffer(std::vector<char> &buffer)
{
    try
    {
        ...
        if (packetSize > buffer.size())
            throw PacketFactoryError(
                "PacketFactoryError: Something's wrong with packet size, probably incomplete.");
