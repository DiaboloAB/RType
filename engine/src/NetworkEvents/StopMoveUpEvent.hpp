...
{
/**
 * @class StopMoveUpEvent
 *
 * @brief Class used to manage in-game entities when a STOP_MOVE_UP
...
class StopMoveUpEvent
{
   public:
    ...
    /**
     * @brief Updates the client's direction to stop upward movement. Sends the updated direction to
     * other clients to ensure they stop moving this client upward as well.
     *
     * @param sender: Sender endpoint of the game event.
                       ...
                       GameContext &gameContext, NetworkIdHandler &idHandler)
    {
        ...
        for (auto &entity : view)
        {
            ...
            if (networkC.id == senderData->second.getNetworkId())
            {
                ...
                if (allyComp.moveDirection.y > 0.0) return;
