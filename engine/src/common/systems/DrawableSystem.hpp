   ...
   public:
    ...
    void draw(mobs::Registry &registry, GameContext &gameContext) override
    {
        ...
        for (auto entity : view3)
        {
            ...
            if (mousePos.x > transform.position.x && mousePos.x < transform.position.x + button.size.x &&
                mousePos.y > transform.position.y && mousePos.y < transform.position.y + button.size.y)
            {
                mlg::vec4 buttonRect(transform.position.x, transform.position.y, button.size.x,
                                    button.size.y);
                gameContext._runtime->drawRectangle(buttonRect,
                    gameContext._runtime->getKey(KeyCode::Mouse0),
                    button.color);
                if (gameContext._runtime->getKey(KeyCode::Mouse0))
                {
                    try {
                        gameContext.get<CppScriptComponent>(button.target).callAllFunctions(
                            button.action, registry, gameContext);
                    } catch (const std::exception &e) {
                        std::cerr << "Error: " << e.what() << std::endl;
                    }
                }
            }
            gameContext._runtime->drawText(button.font, button.text,
                                           mlg::vec3(transform.position.x + button.size.x / 2 - 5,
                                                      transform.position.y + button.size.y / 2 - 5, 0),
                                                      16, mlg::vec3(255, 255, 255), true);
            // gameContext._runtime->drawButton(button.text, transform.position, button.size, button.color,
