...
class NetworkSystem : public ISystem
{
   public:
    ...
    {
        this->_systemsMap[std::type_index(typeid(Network::HiServerPacket))] =
        ...
        this->_systemsMap[std::type_index(typeid(Network::HiClientPacket))] =
            [this](std::shared_ptr<dimension::APacket> &packet, asio::ip::udp::endpoint &sender,
                   mobs::Registry &registry, GameContext &gameContext)
        { handleHiClient(packet, sender, registry, gameContext); };
        //this->_systemsMap[std::type_index(typeid(AdiosServerPacket))] =
        //    [this](std::shared_ptr<dimension::APacket> &packet, asio::ip::udp::endpoint &sender,
        //           mobs::Registry &registry, GameContext &gameContext)
        //{ handleAdiosServer(packet, sender, registry, gameContext); };
        //this->_systemsMap[std::type_index(typeid(Network::PingPacket))] =
        //    [this](std::shared_ptr<dimension::APacket> &packet, asio::ip::udp::endpoint &sender,
        //           mobs::Registry &registry, GameContext &gameContext)
        //{ handlePing(packet, sender, registry, gameContext); };
        //this->_systemsMap[std::type_index(typeid(Network::CreateEntityPacket))] =
        //    [this](std::shared_ptr<dimension::APacket> &packet, asio::ip::udp::endpoint &sender,
        //           mobs::Registry &registry, GameContext &gameContext)
        //{ handleCreateEntity(packet, sender, registry, gameContext); };
        //this->_systemsMap[std::type_index(typeid(Network::DestroyEntityPacket))] =
        //    [this](std::shared_ptr<dimension::APacket> &packet, asio::ip::udp::endpoint &sender,
        //           mobs::Registry &registry, GameContext &gameContext)
        //{ handleDestroyEntity(packet, sender, registry, gameContext); };
        //this->_systemsMap[std::type_index(typeid(Network::MoveEntityPacket))] =
        //    [this](std::shared_ptr<dimension::APacket> &packet, asio::ip::udp::endpoint &sender,
        //           mobs::Registry &registry, GameContext &gameContext)
       ...
       ////{ handleUpdateEntity(packet, sender, registry, gameContext); };
        //this->_systemsMap[std::type_index(typeid(Network::ClientEventPacket))] =
        //    [this](std::shared_ptr<dimension::APacket> &packet, asio::ip::udp::endpoint &sender,
        //           mobs::Registry &registry, GameContext &gameContext)
        //{ handleClientEvent(packet, sender, registry, gameContext); };
        this->_systemsMap[std::type_index(typeid(Network::PacketValidationPacket))] =
            [this](std::shared_ptr<dimension::APacket> &packet, asio::ip::udp::endpoint &sender,
                   mobs::Registry &registry, GameContext &gameContext)
                        ...
                        mobs::Registry &registry, GameContext &gameContext)
    {
        ...
        auto packetToSend = this->_factory.createEmptyPacket<Network::HiClientPacket>();
                        ...
                        mobs::Registry &registry, GameContext &gameContext)
    {
        ...
        std::map<asio::ip::udp::endpoint, Network::EndpointState> endpointMap =
            gameContext._networkHandler->getEndpointMap();
        auto packetVal = this->_factory.createEmptyPacket<Network::PacketValidationPacket>();
        packetVal->setPacketReceiveType(packet->getPacketType());
        packetVal->setPacketReceiveTimeStamp(packet->getPacketTimeStamp());
        if (endpointMap.count(sender) == 1)
            gameContext._networkHandler->updateEndpointMap(sender, true);
        gameContext._networkHandler->sendNewPacket(*packetVal, sender);
...
//
    ...
    void manageQueue(mobs::Registry &registry, GameContext &gameContext)
    {
        ...
        while (!packetQueue.empty())
        {
            try
            {
                ...
                auto typeIndex = this->_factory.getIndexFromType(type);
                this->_systemsMap[typeIndex](packet, sender, registry, gameContext);
            }
            catch (std::exception &e)
            {
   ...
   private:
    std::unordered_map<std::type_index, std::function<void(std::shared_ptr<dimension::APacket> &,
