...
RenderSystemSDL::RenderSystemSDL() : _isFullScreen(false), _nextSpriteId(1), _nextFontId(1), _nextMusicId(1), _nextSoundId(1) {
    if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO) < 0) {
        std::cerr << "Erreur SDL: " << SDL_GetError() << std::endl;
        return;
    }

    _window = SDL_CreateWindow("RType", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1920, 1080, SDL_WINDOW_SHOWN);
    if (!_window) {
...
bool RenderSystemSDL::getKeyUp(KeyCode key) {
    return _previousKeys[key] && !_currentKeys[key];
}
...
std::shared_ptr<SDL_Texture> RenderSystemSDL::loadTexture(const std::string& filePath) {
    ...
    if (!rawTexture) {
        std::cerr << "Erreur lors du chargement de la texture : " << filePath << " - " << IMG_GetError() << std::endl;
        return nullptr;
    }
...
void RenderSystemSDL::unloadSprite(int spriteId) {
    _spriteCache.erase(spriteId);
}

void RenderSystemSDL::drawSprite(int spriteId, mlg::vec3 position, mlg::vec4 spriteRect, mlg::vec3 scale, float rotation) {
    ...
    if (it != _spriteCache.end()) {
        ...
        SDL_RenderCopyEx(_renderer, it->second.get(), &srcRect, &dstRect, rotation, nullptr, SDL_FLIP_NONE);
    } else {
...
void RenderSystemSDL::drawRectangle(mlg::vec4& spriteCoords, bool full, const mlg::vec3& color) {
    ...
    SDL_SetRenderDrawColor(_renderer, static_cast<Uint8>(color.x), static_cast<Uint8>(color.y), static_cast<Uint8>(color.z), 255);
...
void RenderSystemSDL::drawText(int fontID, const std::string& textStr, const mlg::vec3 position, unsigned int fontSize, const mlg::vec3& color, bool centered) {
...
KeyCode RenderSystemSDL::convertSDLKeyToKeyCode(SDL_Keycode key) {
    switch (key) {
        ...
        case SDLK_MINUS: return KeyCode::Tiret;
