...
int RenderSystemSDL::generateUniqueId() { return currentId++; }
...
bool RenderSystemSDL::getKeyUp(KeyCode key) { return false; }

bool RenderSystemSDL::getKeyDown(KeyCode key) { return false; }
...
void RenderSystemSDL::updateWindow() { SDL_RenderPresent(renderer); }
...
void RenderSystemSDL::drawSprite(int id, mlg::vec3 position, mlg::vec4 spriteRect, mlg::vec3 scale,
                                 float rotation)
{
    auto texture = textures[id];
    if (!texture) return;
...
bool RenderSystemSDL::isWindowOpen() { return windowOpen; }
...
void RenderSystemSDL::unloadSound(int id) { soundEffects.erase(id); }
