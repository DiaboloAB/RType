...
Room::Room(const std::shared_ptr<PacketFactory> &factory, std::string host, unsigned int port, 
    std::string code) : AEndpoint(factory), _host(host), _port(port), _code(code)
{
    try {
...
void Room::pingEndpoints() {
    ...
    if (std::chrono::duration_cast<std::chrono::milliseconds>(actualTime - this->_lastPing).count()
        >= 500) {
